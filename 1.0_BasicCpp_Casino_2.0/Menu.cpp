...
namespace menu {
	...
	char GetUserOption() {
		...
		std::cout << "	Press [5] :: Play Roulette " << '\n';
		std::cout << "	Press [6] :: Display statistics " << '\n';
	...
	void RunCasino() {
		...
		while (isGameActive) 
		{
			...
			case static_cast<char>(Optiones::Guess):
				guess::RunGuess(currentBudget, playerBet, isGameActive, statisticLst);
				break; 
			case static_cast<char>(Optiones::OddOrEven):
				oddOrEven::RunEvenOrOdd(currentBudget, playerBet, isGameActive, statisticLst);
				break;
			case static_cast<char>(Optiones::Bar_Dice):
				bar_dice::RunBar_Dice(currentBudget, isGameActive, statisticLst);
				break;
			case static_cast<char>(Optiones::Crown_and_Anchor):
				crownAndAnchor::RunCrownAndAnchor(currentBudget, playerBet, isGameActive, statisticLst);
				break;
			case static_cast<char>(Optiones::Roulette):
				roulette::RunRoulette(currentBudget, playerBet, isGameActive, statisticLst);
				break;
			case static_cast<char>(Optiones::ScoreBoard):
				scoreboard::RunBoard(statisticLst);
				break;
			}
